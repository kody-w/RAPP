...
echo ""
echo "--- pack ---"
TWINS_HOME=$SRC bash hippocampus/twin-egg.sh pack --out $EGG > /tmp/egg-pack.out
...
echo ""
echo "--- info ---"
INFO=$(bash hippocampus/twin-egg.sh info $EGG)
...
echo ""
echo "--- unpack ---"
TWINS_HOME=$DST bash hippocampus/twin-egg.sh unpack $EGG --into $DST > /tmp/egg-unpack.out
