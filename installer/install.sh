...
# at ~/.brainstem on a different port.
DIR="\$(cd "\$(dirname "\$0")" && pwd)"
cd "\$DIR/.brainstem/src/rapp_brainstem" && \\
    PORT=$LOCAL_PORT exec "\$DIR/.brainstem/venv/bin/python" $LAUNCHER
EOF
    chmod +x "$START_SH"

    GIT_IGNORE="$(pwd)/.gitignore"
    if [ ! -f "$GIT_IGNORE" ] || ! grep -qxF '.brainstem/' "$GIT_IGNORE" 2>/dev/null; then
        echo '.brainstem/' >> "$GIT_IGNORE"
        echo 'start.sh' >> "$GIT_IGNORE"
    fi

    echo ""
    echo -e "${GREEN}✓ ready${NC}  run ${CYAN}./start.sh${NC}   ${YELLOW}(project-local · port $LOCAL_PORT · ~/.brainstem untouched)${NC}"
    echo ""
    exit 0
fi
