...
async function fillMutations() {
  ...
  try {
    ...
    if (!r.ok) { list.innerHTML = '<li class="skel">no mutation history yet</li>'; return 0; }
    ...
    // Total mutation count — return the page count for achievement gating.
    return commits.length >= 6 ? 25 : commits.length;
  } catch (_) {
...
// (operator steering), and custom skills (capabilities earned beyond
// the doorman defaults).
//
// All inputs are sub-linear (sqrt-dampened on the saturating signals
// time + mutations + memories) so an organism with 10 000 memories
// reads as proportionally experienced rather than 250 × stronger
// than one with 40. MMR caps at ~9000 for "Immortal" but doesn't
// truly ceiling — there's always more growth available.
//
...
function computeMMR({ memCount, mutCount, customSkills, ageDays }) {
...
function fillMMR(inputs) {
  ...
  bd.textContent = parts.length ? "earned from " + parts.join(" · ") : "fresh plant — score grows as it lives, learns, and earns mutations";
}
...
(async function fillTrackRecord() {
  // Memory count + age first (we'll need them for achievements + MMR)
  let memCount = 0, plantedAt = null;
  try {
    const r = await fetch(".brainstem_data/memory.json", { cache: "no-cache" });
  ...
  try {
    const r = await fetch("rappid.json", { cache: "no-cache" });
    if (r.ok) { const j = await r.json(); plantedAt = j.planted_at || null; }
  } catch (_) {}

  // Skills + mutations + lineage in parallel; achievements + MMR need mut count
  const [, mutCount, customSkills] = await Promise.all([
    fillSkills(),
    fillMutations(),
    fillLineage().then(() => _customSkillCount()),
  ]);
  const ageDays = plantedAt ? (Date.now() - new Date(plantedAt).getTime()) / 86400000 : 0;
  fillMMR({ memCount, mutCount, customSkills, ageDays });
