...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
  ...
  /* Propose-a-skill pane — agent submission form */
...
<main>
  ...
  <div class="row-actions">
    ...
    <button class="action secondary" id="btn-propose-skill" title="Submit a new agent.py to this organism via PR — the lineage evolution path.">🌱 Propose a skill</button>
  ...
  <!-- Propose a skill — the PR-driven evolution path. The frozen kernel
       never moves; capabilities grow via agent .py files merged into
       the seed's /agents/. Visitor fills out the form, we open the
       GitHub create-file URL in a new tab pre-filled with their code;
       GitHub auto-handles fork + branch + PR for non-collaborators. -->
  <section class="pane" id="pane-propose" hidden>
    <h2>🌱 Propose a new skill</h2>
    <p>The frozen kernel never changes. Capabilities grow via <code>agent.py</code> files merged into <code>/agents/</code>. If you found a pattern useful working with this organism, package it as an agent and submit it back. Two paths, your choice:</p>
    <ul style="font-size:13px;color:#8b949e;line-height:1.6;margin:0 0 14px 18px;">
      <li><strong>Rejoin the lineage</strong> — open a PR; if the operator merges it, your skill becomes part of this organism on every visitor's next visit.</li>
    ...
    <div class="propose-row">
      <label class="propose-label">Skill name <span class="propose-hint">(snake_case)</span></label>
...
async function buildDoormanEgg() {
  ...
  if (oc.owner && oc.repo) {
    ...
    const customSkillsSnap = counts.agents > 2 ? counts.agents - 2 : 0;  // ManageMemory + ContextMemory are baseline
    const mmrAtSeal = computeMMR({
      memCount: memSnapshot, mutCount: mutCountSnap,
      customSkills: customSkillsSnap, ageDays: ageDaysAtSeal,
    ...
    stateAtSeal = {
      ...
      custom_skill_count: customSkillsSnap,
...
// ── Propose a skill (the lineage-evolution path) ───────────────────
//
// Capabilities grow via PRs of new agent .py files into /agents/. The
...
// the skill becomes part of the lineage on the next page render.
...
function _agentTemplate(name, description, organismName) {
  ...
  return `"""
${cls}: ${description || "<one-line description of what this skill does>"}

Proposed back to ${organismName || "this organism"} on ${new Date().toISOString().slice(0, 10)}
via the front-door 'Propose a skill' flow.
"""
...
function submitProposedSkill() {
  ...
  if (!name) { alert("Give the skill a name first (snake_case, no spaces)."); return; }
  ...
  u.searchParams.set("message", `propose new skill: ${slug}`);
  u.searchParams.set("description", desc || `New agent proposed back to this organism via the front-door 'Propose a skill' flow.`);
...
document.getElementById("btn-propose-skill").onclick = showProposePane;
document.getElementById("btn-propose-submit").onclick = submitProposedSkill;
...
async function fillSkills() {
  ...
  // An organism running offline still renders its skills from the last
  ...
  if (!Array.isArray(list)) {
    el.innerHTML = res.source === "none"
      ? '<span class="skill-chip skel">offline — skills will load when network returns</span>'
      : "";
    return;
  }
  const skills = [];
  ...
  const stalePill = (res.source === "cache" && res.stale)
    ? `<span class="skill-chip skel" title="last synced ${res.fetchedAt ? res.fetchedAt.toLocaleString() : 'a while ago'} — offline reading from cache">📡 stale</span>`
    : "";
...
async function _parentLineageGift(rappidObj) {
  ...
  if (Array.isArray(agentsRes.value)) {
    customSkills = agentsRes.value.filter(f =>
