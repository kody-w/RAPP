...
<main>
  ...
  <section class="pane" id="pane-propose" hidden>
    ...
    <p>The frozen kernel never changes. Capabilities grow via <code>agent.py</code> files merged into <code>/agents/</code>. If you found a pattern useful working with this organism, package it as an agent and submit it back — the operator reviews + merges; once accepted, it becomes part of the lineage.</p>
...
async function buildDoormanEgg() {
  ...
  if (oc.owner && oc.repo) {
    try {
      const r = await fetch(`https://api.github.com/repos/${oc.owner}/${oc.repo}/commits?per_page=1`, {
