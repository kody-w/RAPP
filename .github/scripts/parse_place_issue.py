...
def validate(parsed: dict[str, str]) -> tuple[bool, list[str]]:
    ...
    if slug and not re.fullmatch(r"[a-z0-9][a-z0-9_-]*", slug):
        errors.append(f"slug not a valid kebab-style identifier: {slug!r}")

    coords = parsed.get("coordinates", "")
    if coords:
        # Lenient — allow whitespace, decimal points, optional sign, comma separator
        if not re.fullmatch(r"\s*[-+]?\d+(\.\d+)?\s*,\s*[-+]?\d+(\.\d+)?\s*", coords):
    ...
    if total == 0:
        errors.append("eligibility section not found in issue body")
    elif checked < total:
