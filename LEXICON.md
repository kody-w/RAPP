...
| An egg packing several twins together so a federation can be resurrected on another machine | **neighborhood egg** | **neighborhood-scale egg** (`scale: neighborhood` + `members.json` roster; see [[Neighborhood Egg — Snapshot and Hatch]] for the pattern doc, [[NEIGHBORHOOD_EGG_SPEC]] for the wire format) |
...
| Hatching a neighborhood egg with the peers re-mapped onto the local box | **local-simulate** | **`target=local-simulate`** on `NeighborhoodRun.hatch` — peer twins extract to `~/.rapp/simulated/<peer>/twins/<hash>/`; no SSH; full offline replay |
