...
# commit: da6cb94985c9525b681bc20c2926656bdfdad565
# blob: eafd5c92af6cc275bfd3e3c16ee4a9a95c43eb97
# sha256: faafb112ff6eb2a9d145479a018e2e5dbffa276185df6f87ddbb2c924d215a52
historical_twin_egg() (
set -e
set -o pipefail
...
print_plan() {
    printf '%s\n' \
        '{"schema":"rapp-twin-egg-plan/1.0","mode":"plan","legacy_schema":"rapp-egg/1.0","operations":["enumerate-twin-workspaces","exclude-transient-runtime-state","hash-members","write-deterministic-zip","verify-safe-extraction"],"packaging":false,"extraction":false,"historical_source_commit":"da6cb94985c9525b681bc20c2926656bdfdad565","effects":[],"accepted":false}' \
        'The legacy archive algorithm is preserved as migration evidence; no archive is opened, created, extracted, started, or accepted in plan mode.'
}

refuse_package() {
    printf '%s\n' \
        '{"schema":"rapp-effect-refusal/1.0","operation":"legacy-twin-egg-package-or-extract","code":"authenticated-registry-unavailable","effects_started":false,"requirements":["reviewed-dependency-injection","exact-target-receipt","authenticated-fresh-section-13-evidence","registered-rapp-1-egg-variant"]}' \
        'Current authority cannot authenticate or register this legacy aggregate; refusal occurs before zip creation, extraction, chmod, mkdir, or twin launch.' >&2
    return 78
}
...
case "${1:-plan}" in
    plan|--plan|inspect|--inspect|check|--check)
        print_plan
        ;;
    pack|unpack|apply|--apply|run|--run)
        refuse_package
        ;;
    info|list)
        historical_twin_egg "$@"
    ...
    help|-h|--help)
        printf '%s\n' \
            'Usage: twin-egg.sh [--plan|--inspect|--check|info EGG|list|pack|unpack]' \
            'Default: deterministic read-only package/extraction plan.' \
            'info and list are explicit local inspection modes.' \
            'pack/unpack require reviewed dependency injection, an exact target receipt,' \
            'authenticated fresh RAPP/1 section-13 evidence, and a registered variant.'
        ;;
    *)
