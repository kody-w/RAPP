...
# blob: 3e7c04a241c64035b97a6beccadf9f97ff1d3888
# sha256: 19e60871021e7c1ee549ef7b42217f1bf1544d9f7790f5ea9da97aa284802bf4
historical_twin_sim() (
set -e
set -o pipefail
...
print_sandbox_replay() {
    printf '%s\n' \
        '{"schema":"rapp-twin-sandbox-replay/1.0","mode":"sandbox","seed":"restored-twin-sim-v1","facade":"http://127.0.0.1:7073/chat","twins":[{"name":"kody","state":"planned"},{"name":"molly","state":"planned"}],"steps":["derive-deterministic-local-identities","plan-loopback-workspaces","plan-mutual-peer-receipts","route-candidate-chat-through-rapp1-facade","derive-local-twin-presentation"],"effects":[],"accepted":false}' \
        'Sandbox replay is deterministic and performs no mkdir, bind, process launch, signal, network, model, repository, or file mutation.'
}

refuse_run() {
    printf '%s\n' \
        '{"schema":"rapp-effect-refusal/1.0","operation":"twin-simulator-run","code":"authenticated-registry-unavailable","effects_started":false,"requirements":["reviewed-dependency-injection","exact-target-receipt","authenticated-fresh-section-13-evidence"],"required_facade":"http://127.0.0.1:7073/chat"}' \
        'Current authority is unavailable; legacy start/stop/peer/wipe/demo/open/logs execution is refused before any subprocess, bind, network call, model call, signal, or mutation.' >&2
    return 78
}
...
case "${1:-sandbox}" in
    sandbox|--sandbox|plan|--plan|inspect|--inspect|check|--check)
        print_sandbox_replay
        ;;
    run|--run|apply|--apply|start|stop|peer|wipe|demo|open|logs|list|ls)
        refuse_run
        ;;
    help|-h|--help)
        printf '%s\n' \
            'Usage: twin-sim.sh [--sandbox|--plan|--inspect|--check|--run]' \
            'Default: deterministic in-memory replay targeting the exact loopback facade.' \
            'Historical commands retained: start, stop, list, logs, open, peer, wipe, demo.' \
