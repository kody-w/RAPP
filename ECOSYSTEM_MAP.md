...
| `.well-known/rapp-network.json` (per published estate) | Per-operator beacon emitted by `estate publish`. Carries operator rappid, estate URL, protocol versions implemented, `discovery.indexable` consent flag (robots.txt-style; default true), and `discovery.federation_hints[]`. | rapp-network-beacon/1.0 |
