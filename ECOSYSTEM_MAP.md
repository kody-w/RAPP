...
| `tools/backfill_seeds.py` | One-shot operator-tooling: walks known seeds, validates rappid via `door_from_rappid`, reissues invalid rappids, emits missing canonical files (facets.json, members.json, .nojekyll, README). Idempotent. | (no schema; uses gh api PUTs) |
