...
echo "▶ Vendoring rapp_brainstem core into community_rapp/$DEST/"
rm -rf "$DEST"
mkdir -p "$DEST"

# Copy each Python module the function app imports.
# function_app.py still imports as `server` (back-compat alias).
for src in brainstem.py llm.py chat.py t2t.py _basic_agent_shim.py; do
    if [ -f "$ROOT/rapp_brainstem/$src" ]; then
        dest="$src"
        [ "$src" = "brainstem.py" ] && dest="server.py"
...
echo "    cd community_rapp && func azure functionapp publish <APP_NAME> --build remote"
