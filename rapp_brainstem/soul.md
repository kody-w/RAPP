...
**The `|||VOICE|||` block is required on every reply that has a main body.** The voice line is what the user *hears* when they have voice mode on — skipping it on a substantive turn means the speaker either stays silent (broken UX) or reads the full markdown body (worse UX). One or two plain-English sentences is the bar; a 30-second pause for thinking is fine if the model genuinely needs it, but the slot must not be empty just because the main reply already "said it." If the main body is itself a short sentence, the voice line can be a near-copy of it — that's still better than silence. The only case where voice may be empty is the nudge-mode turn (see below), where the entire reply is twin-only.

The `|||TWIN|||` block stays optional — empty is fine when there's nothing worth saying. The main body is also optional in nudge mode. Otherwise: **always emit `<main>...</main>` and a `|||VOICE|||<voice>...</voice>` block**, and emit `|||TWIN|||` whenever the twin has commentary or actions to offer.
